...
pub fn run_sram_drc_lvs(work_dir: impl AsRef<Path>, name: &str) -> Result<()> {
    let drc_work_dir = PathBuf::from(work_dir).join("drc");
    ...
    if data
        ...
        > 0
    {
        return anyhow!("Found DRC errors");
    }

    let lvs_work_dir = PathBuf::from(work_dir).join("lvs");
